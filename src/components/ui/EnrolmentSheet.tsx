...
export const EnrolmentSheet: React.FC<EnrolmentSheetProps> = ({ open, token, expiresAt, onClose }) => {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-[640px] bg-[var(--color-panel)] rounded-2xl overflow-hidden animate-modal-in"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="px-6 py-5 border-b border-[var(--color-ln)] flex items-start justify-between gap-3">
          <div>
          ...
          <button type="button" onClick={onClose} className="w-[30px] h-[30px] grid place-items-center rounded-lg text-[var(--color-tx3)] hover:bg-[var(--color-ln2)]">
            <X className="w-4 h-4" />
          </button>
        </div>
        <div className="px-6 py-5">
          ...
          <div className="flex items-center gap-2.5 px-4 py-3.5 rounded-xl bg-slate-950">
            ...
            {expiresAt && <span className="tnum text-[11px] text-[var(--color-tx2)] shrink-0">{expiresAt}</span>}
            <button
              type="button"
          ...
          <div className="flex gap-0.5 mt-5 border-b border-[var(--color-ln)]">
            {(['Windows', 'Linux', 'macOS'] as const).map((tab) => (
              <button
                ...
                className={`px-3.5 py-2.5 border-0 bg-transparent text-[12.5px] font-semibold border-b-2 ${
                  os === tab ? 'border-[#D0B335] text-[var(--color-tx)]' : 'border-transparent text-[var(--color-tx2)]'
                }`}
              >
                {tab}
              </button>
          ...
          <div className="mt-4">
            <div className="text-[11px] font-semibold text-[var(--color-tx3)] mb-2">Commande d'installation</div>
            <div className="flex items-start gap-2.5 p-4 rounded-lg bg-[var(--color-ln2)] border border-[var(--color-ln)]">
              <code className="tnum flex-1 text-xs leading-relaxed text-[var(--color-tx2)] break-all font-mono">{cmds[os]}</code>
          ...
          <div className="flex items-center gap-3 mt-5 px-4 py-3 rounded-xl border border-dashed border-slate-300">
            ...
            <span className="text-[12.5px] font-semibold text-[var(--color-tx2)]">En écoute du premier heartbeat…</span>
            <span className="tnum text-[11.5px] text-[var(--color-tx3)] ml-auto">aucun agent détecté</span>
          </div>
        </div>
        <div className="px-6 py-4 border-t border-[var(--color-ln)] bg-[var(--color-ln2)] flex justify-end">
